...
public class UserController {
	...
	public ResponseEntity<Object> updatePass(@RequestParam("id") Integer id, @RequestParam("oldpass") String oldpass,
			@RequestParam("newpass") String newpass) {
		User user1;
		user1 = userService.getUserById(id);
		BCryptPasswordEncoder bCryptPasswordEncoder = new BCryptPasswordEncoder();
		if (bCryptPasswordEncoder.matches(oldpass, user1.getPassword()) == true) {
	...
	public List<User> sendFriendInvitation(@PathVariable("id1") int id1, @PathVariable("id2") int id2,
			@PathVariable(value = "textsearch", required = false) String textSearch) {
