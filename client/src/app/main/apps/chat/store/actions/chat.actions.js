...
const getChatData = (contactId, user) => {
    let response;
    ...
    axios
        .get(`${endPointApi.chat.getChat}${chatId}`)
        .then((res) => {
            if (res.status == 200) {
                response = res.data;
    ...
    if (!chat) {
        axios.get(endPointApi.users.fetchById + user.id)
        .then(response => {
...
export function getChat(contactId) {
    return (dispatch, getState) => {
        ...
        const { chat, userChatData } = getChatData(contactId, user);
